...
try:
    ...
    raise AssertionError(
        "В приложении `blog` опишите " "модели `Post, Category, Location`"
    )
except RuntimeError:
    ...
    if not set(need_apps.values()).intersection(registered_apps):
        need_apps = {"blog": "blog.apps.BlogConfig",
...
pytest_plugins = ["fixtures.fixture_data"]
...
def post_context_key(user_client, post_with_published_location):
    ...
    for key, val in dict(post_response.context).items():
        if isinstance(val, Post):
            post_key = key
            break
    assert post_key, "Убедитесь, что в контекст страницы поста передан пост."
    return post_key
